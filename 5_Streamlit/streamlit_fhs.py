...
def create_wordcloud(topic):
    word=list(pd.read_csv('/app/BDCoders_Abandono_escolar/streamlit/wordclouds/words_'+str(topic)+'.csv')['0'])

    wordcloud = WordCloud(max_font_size=50, max_words=100,font_path="/app/BDCoders_Abandono_escolar/streamlit/fonts/arial/arial.ttf", background_color="white").generate(' '.join(word))
    return wordcloud
